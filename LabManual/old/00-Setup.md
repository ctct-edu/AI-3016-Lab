...
## タスク1 : Microsoft Foundryへのアクセス

このタスクでは、Microsoft AzureにおけるAIサービスを統合的に管理するMicrosoft Foundryへアクセスします。
各演習ではMicrosoft Foundry上で演習に使用するリソースを作ります。
...
2. https://ai.azure.com/managementCenter/allResources へアクセスし、以下のアカウントでサインインします。
   ...
   | 項目       | 値                                        |
   | ---------- | ----------------------------------------- |
   | ユーザーID | `AI3016StudentXX@ctctedu.onmicrosoft.com` |
   | パスワード | 講師から配布されたパスワード              |

    >**注:** ツアーやチュートリアルが表示される場合は、"キャンセル"もしくはポップアップの×ボタンをクリックして閉じます。
    >
    >Microsoft Foundryの言語が英語になっている場合は、ログイン後の画面上部右側"歯車アイコン"をクリックし、"Language"から"日本語(Japanese)"を選択することで変更可能です。

## タスク2 : 演習リソース作成

このタスクでは、演習で使用するリソースのために**プロジェクトとハブリソース** を作成します。 Microsoft Foundry では、プロジェクトを作業の整理を行うためのコンテナーとして使用します。
...
高度なオプションを展開して以下のパラメーターを設定します。
   ...
   | パラメーター     | 値                                    |
   | ---------------- | ------------------------------------- |
   | リソースグループ | **AI3016StudentXX**（XXは自身の番号） |
...
1. プロジェクトの作成が完了すると、 **Microsoft Foundry** へ強制的に遷移します。
